...
// [added] backwards compatability with an old api version
// #define AV_CODEC_CAP_OTHER_THREADS AV_CODEC_CAP_AUTO_THREADS

// [added] moved from elsewhere, and commented out if
// #if !CONFIG_SWSCALE
...
void exit_program(int ret)
{
    ...
    printf("FFMPEG exit_program called! (cmdutils.c, line 138)");
    exit(ret);
}
...
void show_help_children(const AVClass *_class, int flags)
{
    ...
    // [added] removed these 2 lines to be compatible with the new api. Hopefully it's not important.
    // while (child = av_opt_child__class_iterate(_class, &iter))
...
int opt_cpucount(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!ret) {
        // [added] replaced directly with the new api implementation
        // av_cpu_force_count(count);
        atomic_int tmp = av_cpu_count();
        atomic_store_explicit(&tmp, count, memory_order_relaxed);
    }
...
int show_filters(void *optctx, const char *opt, const char *arg)
{
    ...
    while ((filter = av_filter_iterate(&opaque))) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            // [added] make backwards compatible with the old api version
...
static void show_help_filter(const char *name)
{
    ...
    printf("    Inputs:\n");
    // [added] make backwards compatible with the old api. Hopefully this doesn't break anything.
    count = -1; // count = avfilter_filter_pad_count(f, 0);
    for (i = 0; i < count; i++) {
    ...
    printf("    Outputs:\n");
    // [added] make backwards compatible with the old api. Hopefully this doesn't break anything.
    count = -1; // count = avfilter_filter_pad_count(f, 1);
    for (i = 0; i < count; i++) {
